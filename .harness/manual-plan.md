...
- [x] **창이 뜨는가 — 실행 → 창 표시는 확인됐다** (2026-08-05 실물). 페인 둘·Details·
      ...
      **남은 절반**: 두 번째 실행에 폴더 인자를 주면(`FlexDir.Host.exe C:\Windows`)
      그 폴더를 활성 페인에서 여는가.
...
- [ ] **주소줄에 경로 자동완성 드롭다운이 뜬다는 보고** (2026-08-06) — **우리 기능이
      아니다** (주소줄은 순수 TextBox, 자동완성 코드 없음). 외부 유틸(Listary 류)이나
...
**아직 사람이 봐야 하는 것** (키보드·체감이라 캡처로 판정할 수 없다):

- [ ] **type-ahead** — 목록에 포커스를 두고 문자를 치면 그 이름으로 점프하는가. 1초 지나면
      접두어가 리셋되는가. 주소줄에 포커스가 있을 때는 주소줄에 글자가 들어가는가.
- [ ] **`Ctrl+L`·`Alt+D`** — 그 페인 주소줄에 포커스가 가고 전체 선택되는가. 좌우 페인이
      각자 자기 주소줄로 가는가.
- [ ] **뷰 3종의 키보드 이동** — 목록 뷰에서 `↓` 가 같은 열 아래, `→` 가 다음 열인가
      (DESIGN §9). 타일·큰 아이콘에서 `↓` 가 한 열 수만큼 뒤인가. PageUp/Down 이 한 화면인가.
- [ ] **wrap 뷰의 클릭** — 칸을 누르면 그 항목이 선택되고, 마지막 줄의 남은 칸을 누르면
      선택이 풀리는가 (그렇게 만들었다).
- [ ] **대용량 폴더 스크롤 체감** — `C:\Windows\WinSxS` 를 큰 아이콘으로 훑을 때 매끄러운가.
      썸네일이 스크롤을 따라오는가, 지나간 것이 해제되는가.
- [ ] **`VirtualizingPanel.CacheLength` 는 기본값(앞뒤 1페이지)이다.** 보이는 것의 약 3배를
      실현하므로 썸네일도 그만큼 요청한다. 96px 가 항목당 최대 256KB 라 큰 폴더에서
      메모리를 봐야 한다. 문제가 되면 `CacheLength` 를 줄인다.
- [ ] **`DESIGN.md` §7 의 큰 아이콘 글리프 `E8A6` 은 틀렸다** — 그 자리는 잠긴 문서
      (ProtectedDocument)다. 큰 사각 하나(`E15B`)로 바꿔 뒀다. 확정되면 §7 표를 고친다.
