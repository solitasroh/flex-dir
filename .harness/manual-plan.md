...
- [x] ~~**최대화가 작업영역과 정확히 맞는다.**~~ **2026-08-12 에 뒤집혔다 — 틀린 판정이었다.**

  당시 근거는 `PrintWindow` 픽셀이었는데 그것으로는 잘림을 볼 수 없다 (§확인 도구의 ⛔).
  실제로는 바깥 8px 이 화면 밖이라 32px 캡션 줄이 24px 만 보였다. `MaximizedFrame` 이
  덜어내도록 고쳤고(`ad5b06b`), **UIA 로 캡션 글리프가 작업영역 위끝에서 8px 인 것을
  재서** 확인했다.
...
- [x] ~~**다른 배율·다른 모니터에서의 최대화** — 이상 없다.~~ **2026-08-12 에 뒤집혔다.**

  두 모니터 다 100% 배율이라 배율은 변수가 아니었고, **잘림은 두 모니터 모두에서**
  일어나고 있었다 (위 항목과 같은 원인). 눈으로만 본 smoke 테스트가 8px 을 놓친 자리다.
...
- **그림이 필요할 때만** `PrintWindow` 로 캡처한다. 그 캡처의 바깥 8px 은 창이 아니다
  (§시안 대조 참조).
- ⛔ **`PrintWindow` 로 "잘렸는가" 를 판정하지 마라** (2026-08-12 · 여기서 두 번 틀렸다 —
  2026-08-06 의 §최대화 여백 판정과 이번 세션의 첫 판정 둘 다). 검은 테두리가 **잘려나간
  자리를 정확히 덮어써서**, 내용이 `(8,8)` 에서 시작한 것과 `(0,0)` 에서 시작해 위 8px 이
  화면 밖으로 나간 것이 **비트맵으로 완전히 똑같이 찍힌다.**
  → 가르는 법: **UIA 로 요소의 화면 좌표를 재서 작업영역과 견준다.** 캡션의 16x16 글리프는
  32px 줄에 중앙 정렬이라 위 여백이 8px 이어야 한다 — 최대화에서 그 값이 0 이면 잘린 것이다.
  ```powershell
  # $el = flex-dir 창의 AutomationElement · $mi.work = GetMonitorInfo 의 작업영역
  $top = ($el.FindAll('Descendants', <ControlType.Text>) | 최소 Top).BoundingRectangle.Top
  $top - $mi.work.Top   # 8 이면 정상 · 0 이면 위가 먹혔다
  ```
...
- [x] **제거 → 재설치** (2026-08-07 · 0.1.2). 제어판에 등록된 명령 그대로
      ...
      ⚠ **뒷부분(모니터 복원)은 2026-08-12 에 거짓으로 드러났다** — 보조 모니터에 최대화로
      저장해 두고 다시 켜면 **주 모니터**에 떴다. 창이 뜨기 전에 `WindowState=Maximized` 를
      걸면 WPF 가 주 모니터 기준으로 크기를 정하고 저장한 좌표를 버린다. `ResidentWindow` 가
      `Loaded` 뒤로 미루도록 고쳤다 (`ad5b06b`). 최대화 여부만 보고 모니터를 확인하지 않으면
      이 오판을 그대로 반복한다.
...
## 분할 — 사람 확인 항목 (2026-08-12 · 정본은 `docs/PRD-v2.md` §18)

**기계로 닿는 데까지는 확인했다** (저장소 Debug 빌드 · PrintWindow 캡처):

- [x] **v0.5.0 저장 파일이 2분할로 뜬다** — 옛 형식(`leftTabs`/`rightTabs`) 그대로인
      파일에서 페인 둘이 떴다. **이 기능에서 가장 되돌리기 비싼 자리다**
- [x] **4분할 배치** — 0=좌상 · 1=우상 · 2=우하 · 3=좌하, 가로줄 일직선
- [x] **3분할 배치** — 좌측 세로 전체, 가로 스플리터가 오른쪽 열에만
- [x] **접힌 페인이 재시작을 건너 산다** — `paneCount: 3` 인데 `panes` 는 넷이었고
      4번째 폴더가 그대로였다
- [x] **저장이 새 형식으로 나간다** — `paneCount`·`rowRatio`·`panes` 가 쓰이고
      `leftTabs` 는 더 이상 안 쓴다

**눌러 봐야 아는 것도 전부 닫혔다 (2026-08-12 · 사용자가 밟았다 — "모두 정상동작해"):**

- [x] **툴바 분할 메뉴(⊞) 넷을 실제로 누른다** — 1·2·3·4 가 각각 먹고 체크 표시가 현재
      분할을 가리킨다. **`Command` 가 `null` 인 `MenuItem` 은 정상으로 뜨고 눌리기까지
      한다**(§17 이 값을 치른 자리)는 함정을 이것으로 배제했다
- [x] **탭 줄 우클릭 → '여기서 분할'** — 넷이면 항목이 접힌다
- [x] **탭 줄 우클릭 → '이 페인 닫기'** — 1분할이면 접힌다. `Tag.Tag` 를 두 단 타고
      올라가는 바인딩이 살아 있다
- [x] **3분할에서 가운데 페인을 지목해 닫기** — 지목한 그 페인이 사라진다 (맨 뒤와
      맞바꾸는 수가 실물에서 옳게 보인다)
- [x] **늘려도 자리가 안 튄다** — 1→2 에서 보던 폴더가 왼쪽에 남고, 2→3 에서 1번이,
      3→4 에서 2·3번이 안 움직인다. **프리셋 슬롯 순서를 고른 이유가 실물에서 성립한다**
- [x] **접었다 폈을 때 그대로 돌아온다** (4→1→4)
- [x] **스플리터 둘을 끄는 손맛** — 세로·가로가 만나는 칸 포함
- [x] **회귀 없음** — `Tab`/`F6` 순환 · `Ctrl+Alt+C` 대상 · 탭 기능 전부 ·
      **트리에서 폴더 클릭**(리팩터링 중 실제로 끊겼다가 테스트가 잡은 자리) ·
      `Ctrl+1~4`(정렬)·`Ctrl+Shift+1~4`(보기 모드)
- [x] **재시작 왕복** — 4분할이 그대로 돌아왔고 접힌 페인도 살아 있었다

**분할에 열린 항목은 없다.** v0.6.0 으로 나갔고 설치본이 4분할로 뜨는 것까지 확인했다 (`HISTORY.md` §v0.6.0).

> 1080 세로 모니터의 4분할은 **아직 안 봤다** — 이 확인은 2560 주 모니터에서 했다.
> 열 320×2 + 트리 220 = 860 이라 들어가지만 목록이 쓸 만한 폭인지는 그 화면에서만 안다
> (§규칙 15 가 이 기계의 두 해상도를 경고한다). 매일 쓰며 판정한다.
