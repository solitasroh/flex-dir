...
- [x] **창이 뜨는가 — 실행 → 창 표시는 확인됐다** (2026-08-05 실물). 페인 둘·Details·
      ...
      **남은 절반**: 두 번째 실행(`FlexDir.Host.exe C:\Windows`)이 창을 앞으로 가져오고
      그 폴더를 여는가 · 창을 닫은 뒤 다시 실행하면 창이 다시 오는가 (상주 경로의 핵심).
...
- [ ] **시작 상태 복원** (2026-08-06 배선) — 첫 실행이 사용자 프로필 두 페인으로 뜨는가 ·
      폴더를 옮기고 완전 종료 후 재실행하면 마지막 폴더·스플리터 비율·창 위치/크기
      (최대화 포함)가 돌아오는가.
- [ ] **트레이 아이콘** (2026-08-06 배선 · 사용자 결정) — 알림 영역에 아이콘이 보이는가 ·
      더블클릭/"창 열기" 가 창을 앞으로 가져오는가 · "완전 종료" 로 프로세스가 정말
      끝나는가 (**창이 보이는 채로도** — `ResidentWindow` 의 Closing 취소를 Shutdown 이
      무시하는지가 이 확인의 핵심이다) · 종료 후 아이콘이 남지 않는가.
- [ ] **닫기 = 숨기기** (2026-08-06 배선) — X 로 닫으면 프로세스가 살아 있고
      (`Get-Process FlexDir.Host`), 다시 실행하면 같은 창이 같은 자리에 다시 오는가.
      이전에는 이 경로가 `Show()` 에서 던졌다 — 닫힌 WPF 창은 다시 보일 수 없다.
- [ ] **스플리터 드래그** — 끌면 페인 비율이 따라오고, 끝까지 끌어도 페인이 사라지지
      않는가 (VM 클램프 0.15~0.85 — 넘겨 끈 순간 튕겨 돌아와야 한다).
- [ ] **계측 실측 수치** — 창을 몇 번 여닫고 폴더를 옮긴 뒤
      `%LOCALAPPDATA%\flex-dir\perf.log` 를 연다. WindowShown ≤ 100ms ·
      FirstItem ≤ 150ms 인가 (`over` 줄이 있는가).
- [ ] **아직 없는 것 (다음 세션)** — 아이콘·썸네일 (`SetVisibleRange` behavior, B-3) ·
      type-ahead 배선 (TextInput behavior) · Ctrl+L 주소줄 포커스 · 이름변경 편집기 (B-4) ·
      드래그앤드롭 (B-4) · 제품 아이콘(트레이는 임시 `SystemIcons.Application`) ·
