...
- [x] **다른 배율·다른 모니터에서의 최대화** — 이상 없다. 훅 없이 두기로 한 결정
  ...
  (아래 HANDOFF §최대화 여백)이 이 기계 구성에서는 맞았다.
...
- [ ] **SMB 에서의 `IFolderWatcher`.** `FileSystemWatcher` 는 UNC 에서 버퍼 오버플로와
      조용한 정지가 흔하다. ADR-011 의 자동 갱신 전제가 여기서 깨질 수 있다 —
      **폴더에 파일을 만들어 목록이 따라오는지** 봐야 한다.
      주의: 업무 공유(대외비)라 테스트 파일을 만들 자리를 사용자가 정해야 한다.
- [ ] **느린/끊긴 서버에서의 조작감.** 이 NAS 는 185ms 로 빠르다. VPN 이나 죽은 호스트에서
      멈추는지는 다른 이야기다 (CLAUDE.md §3 이 겨냥한 자리).
- [ ] **파일 조작(복사·이동·삭제)이 UNC 에서 도는가.** 열거만 확인했다.
...
- [x] **툴바 분류 드롭다운** — 사용자가 확인했다 (2026-08-07). 다섯 항목이 뜨고 `없음` 에
      ...
      찾지 못했고 실제로 뜨지도 않았다 (HANDOFF §진입점을 하나만 둔 것이 틀렸다).
