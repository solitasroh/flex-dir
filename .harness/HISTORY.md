...
| **v0.8.5** | **2026-08-24** | **외부 도구 §21 — `[VS]`·`[>_]` · 포트 둘 · 터미널 설정 · ADR-022** | **서명본** | **구웠고 설치했다** (`-NoUpload` · 지문 `81DB29…`). 0.8.4 → 0.8.5 로 교체되고 창이 떴다. 업로드·푸시는 안 했다. 아래 전문 |

### v0.8.5 — 외부 도구가 들어왔다. **굽고 설치까지 했다** (2026-08-24)
...
⚠ **위에서 설치까지 확인한 `0.8.5+c87bb09…` 설치본은 이 수정 전 빌드다.** 다시 구워
설치하기 전까지는 이 기계의 설치본에 그 크래시 사슬이 그대로 있다.
