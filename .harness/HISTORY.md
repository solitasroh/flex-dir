...
| **v0.8.5** | **2026-08-24** | **외부 도구 §21 — `[VS]`·`[>_]` · 포트 둘 · 터미널 설정 · ADR-022** | **—** | ⛔ **아직 굽지 않았다.** `Directory.Build.props` 를 `0.8.5` 으로 올렸을 뿐이고 `pack.ps1` 은 안 돌렸다 — 굽기·업로드는 사람 확인과 함께 사용자가 한다. 아래 전문 |

### v0.8.5 — 외부 도구가 들어왔다. **버전만 올렸고 굽지 않았다** (2026-08-24)

**적을 것은 버전을 올렸다는 것과 무엇이 들어갔는가다.** 패키지는 이 시점에 존재하지 않는다 —
`pack.ps1` 은 돌리지 않았고, 굽기·서명·업로드는 사람 확인과 함께 사용자가 한다
(`HANDOFF.md` §배포 절차 ④). **배포 결과 칸은 그 뒤에 채운다.**
