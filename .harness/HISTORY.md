...
| **v0.9.0** | **2026-08-24** | **외부 도구 §21 — `[VS]`·`[>_]` · 포트 둘 · 터미널 설정 · ADR-022** | **—** | ⛔ **아직 굽지 않았다.** `Directory.Build.props` 를 `0.9.0` 으로 올렸을 뿐이고 `pack.ps1` 은 안 돌렸다 — 굽기·업로드는 사람 확인과 함께 사용자가 한다. 아래 전문 |

### v0.9.0 — 외부 도구가 들어왔다. **버전만 올렸고 굽지 않았다** (2026-08-24)
