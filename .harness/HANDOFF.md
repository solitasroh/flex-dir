...
>
> **그래서 세션을 끝낼 때 앱을 띄워 둔 채로 두지 않는다** (2026-08-07). Stop 훅이
> `verify.fast` 를 돌리므로, 확인이 끝났다고 도그푸딩용으로 다시 띄우면 **그 훅이 잠긴
> 산출물 때문에 깨진다.** 도그푸딩은 사람이 직접 띄워서 잇는다 — 저장소 밖에서 실행하는
> 방법이 생기기 전까지는 이 둘을 동시에 만족시킬 수 없다.
