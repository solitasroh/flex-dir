...
1구간 v0.8.0). 좁은 페인에서 툴바가 잘리던 것을 `FoldOrder` 무리 접힘 + `»` 메뉴로 받고
(`ADR-021` · `Views/ToolbarOverflowPanel.cs`), 알려진 폴더 다섯(홈·바탕화면·문서·다운로드·
사진, `E707`)이 드롭다운으로 들어왔다. 포트가 하나 늘어 **17/17** (`IKnownFolderList`).
게이트 4종 ✅ · **패키지는 구웠고 업로드는 안 했다** (`-NoUpload`) — 서명 설치본이 실제로
뜨는지의 사람 확인이 남아 있다. 2구간(v0.9.0)은 외부 도구다 (아래 §다음 작업).
...
2026-08-21 에 알려진 폴더가 하나를 더 늘렸다 (§20).
...
### 0. 2구간(v0.9.0) — 외부 도구. **다음 축이다**

1구간(§20)이 `FoldOrder 1` 자리를 이것을 위해 비워 뒀다 (`ADR-021`).

- **포트 둘이 늘어난다** — `IExternalToolCatalog`(설치된 도구 탐지) ·
  `IExternalToolLauncher`(실행). 탐지와 실행을 가르는 것은 열거/조작을 가른 v1 의 수와 같다.
- **`[VS]`** — App Paths 의 `code.exe` 로 탐지해 현재 폴더를 VS Code 로 연다.
- **`[>_]`** — 터미널. **프리셋 5 + 사용자 지정**, 명령줄의 `{path}` 를 현재 폴더로 치환한다.
- **`\\server` 루트에서만 비활성** — 공유 루트는 작업 디렉터리가 될 수 없는 유일한 자리다.
