...
버전     0.8.3   릴리스는 v0.1.0 ~ v0.7.0 까지 나갔다. **v0.8.0~v0.8.3 은 패키지만
...
- **📍(`E707`) 실루엣 판정** — 실제 크기 16px 툴바 맥락에서 눈에 멈추는가 (`DESIGN.md` §7).
  실물을 보니 **압정/사람 실루엣으로 읽힌다** — 계열 구분(선·사각)은 확실하나 뜻이 바로
  오지는 않는다. 바꿀지는 사용자 판단
- **페인 우클릭 크래시 없음** — ⚠ **자동화로 못 밟았다.** `SetForegroundWindow` 와
  `mouse_event` 로 세 번 시도했으나 포그라운드가 안 잡혀 컨텍스트 메뉴가 뜨지 않았다
  (사용자가 기계를 쓰는 중이면 합성 클릭이 계속 밀린다). **사람이 직접 우클릭해야 한다**
