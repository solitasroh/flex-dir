...
**v1 · v1.1 · v2 네트워크 · 배포까지 끝났고, 도그푸딩 게이트는 폐기됐다** (2026-08-07).
남은 것은 **쓰다가 걸리는 것을 GitHub 이슈로 남기는 것**이다 — 게이트가 세던 자리를
그것이 대신한다 (ADR-007 §폐기).

**이번 세션(2026-08-07 오후)이 한 것**: 스플리터 결함 하나(스크롤바 드래그가 비율을
덮었다) · README · 게이트 폐기 · 0.1.2 릴리스 · 배포 확인 둘(`나중에` · 제거→재설치).
자세한 것은 아래 §이번 세션.
...
```
브랜치   main  ·  origin/main 과 동기 (e1f2770 까지 푸시됨)
버전     0.1.2   릴리스 v0.1.0 · v0.1.1 · v0.1.2, 이 기계에 0.1.2 설치됨
테스트   1385 통과   Core 488 · App 572 · Shell 251 · Host 74
...
**v2 네트워크 N-1~N-4 와 인스톨러·자동 업데이트가 들어왔다** (2026-08-07).
남은 것은 **매일 쓰기**와, 사람 손이 필요한 확인 넷이다 (전부 `manual-plan.md` 에 사유와
함께 있다):
...
**이제 도그푸딩은 설치본으로 한다.** 시작 메뉴의 `flex-dir` 이 그것이고, 저장소 산출물을
잠그지 않아 **켜 둔 채로 게이트가 통과한다.**
