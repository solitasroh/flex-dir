...
```
브랜치   **main 에 PR #3이 squash merge됐다** (2026-09-07).
                 태그 v0.8.5는 병합 커밋 `2211c0c`를 가리킨다
...
버전     0.8.5   ✅ **v0.8.5 공개 릴리스 완료** (2026-09-07).
                 `pack.ps1 -SignThumbprint`로 6개 자산을 GitHub에 올렸고,
                 이 기계 설치본은 `0.8.5+2211c0c…`이다. 창 표시와 새 오류 없음 확인.
...
게이트   fast (build -warnaserror · test --blame-hang · check-structure) ✅
         full (Release build -warnaserror) ✅ **2026-09-07 · 병합 직전 HEAD에서 넷 다
         다시 돌렸다. 테스트 2280 · 실패 0 · 경고 0.**
