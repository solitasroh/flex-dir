...
자리는 이제 번호다 (`ADR-019`). **아직 배포하지 않았다.**
...
작업트리 **분할 구간이 통째로 미커밋이다** (2026-08-12). 새 파일 4
                 (`Views/SplitLayout.cs` · `SplitLayoutTests` · `WorkspaceSplitTests` ·
                  `WorkspaceTestSupport`) + 수정 약 20 (Core·Shell·App·Host·docs).
                 **커밋하지 않았다 — 사용자가 요청할 때만 한다** (CLAUDE.md §7)
버전     0.5.0   릴리스 v0.1.0 ~ v0.5.0. **v0.5.0 이 탭과 §16(활성화 채널)을 함께 실어
                 나갔다** (2026-08-12 · `HISTORY.md` §v0.5.0 전문). **이 기계 설치본도 0.5.0 이고
                 실물에서 도는 것을 확인했다.** §16 의 경고는 이제 없다 — 인자 없는 두
                 번째 실행이 활성화 채널을 죽이던 결함이 배포까지 닫혔다.
                 **분할은 아직 안 나갔다** — 다음 배포가 실어 나른다
테스트   1982 통과   Core 569 · App 1010 · Shell 309 · Host 94
                 (분할이 +89. Core 24 · Shell 10 · 배치 판정 21 · 워크스페이스 29 ·
                  스플리터 축 5)
게이트   fast (build -warnaserror · test --blame-hang · check-structure) ✅
         full (Release build -warnaserror) ✅ **넷 다 2026-08-12 미커밋 상태에서 돌렸다.**
...
**네 번 밟았고 네 번 다 이 순서였다** (v0.4.1 에서 굳었다). 개별 배포 기록은
...
**다음은 커밋과 배포다.** 위 §배포 절차 그대로이고, **XAML 루트를 건드렸으므로 설치본이
실제로 뜨는 것까지 봐야 끝난다** (§10 의 `StaticResource` 크래시가 그 자리다).

### 0-1. 탭 — 끝났다. v0.5.0 으로 나갔고 실물에서 돈다 (2026-08-12 갱신)
...
**분할이 끝난 뒤의 축은 도그푸딩이 고른다** (ADR-007 §폐기의 자리). 지금 알고 있는 후보 넷:

| 후보 | 근거 |
|---|---|
| **분할 단축키** | 일부러 안 붙였다 (사용자 결정 2026-08-12) — `Ctrl+1~4` 는 정렬이, `Ctrl+Shift+1~4` 는 보기 모드가 이미 쓴다. **며칠 써 보고 자리를 고른다**가 그 결정의 내용이다 |
...
| §최대화 여백 | `HISTORY.md` §최대화 여백 | 창 rect 가 작업영역 밖으로 사방 8px 나가지만 **보이는 내용은 잘리지 않는다.** `WM_GETMINMAXINFO` 훅은 한 픽셀도 못 바꿔 걷어냈다 |
...
분할               ✅  §18 — 1~4 프리셋 · 기본 1분할 · 접기 · MRU 대상 (ADR-019).
                      ...
                      (2026-08-12). **커밋과 배포가 남았다** (위 §다음 작업 0)
→ 다음 축           ⬜  도그푸딩이 고른다. 후보 넷은 위 §다음 작업 0
```
