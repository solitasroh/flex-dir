...
`DESIGN.md` §7). 게이트 4종 ✅ · 테스트 **2280** · **PR #3 병합과 v0.8.5 공개 배포까지
끝났다** (2026-09-07). 수정 두 건이 든 서명 설치본 `0.8.5+2211c0c…`으로 다시 설치해
창이 뜨고 새 오류가 없는 것도 확인했다. **사람 확인 일곱은 전부 닫혔다.**
⚠ **계획은 v0.9.0 이었고 사용자가 0.8.5 로 정했다** — 실행이 끝난 step 지시서의 `v0.9.0`
표기는 얼어붙은 기록이라 그대로 뒀다 (`docs/PRD-v2.md` §21 머리).

**v0.9.0은 선택과 폴더 트리를 다시 단순하게 만들었다** (2026-09-07). 목록의 `Ctrl+A`가
이름 기반 선택으로 들어가고, 페인 이동 때 트리가 자동으로 따라가던 양방향 배선을 제거했다.
트리 상단에는 `모두 접기`가 생겼고 즐겨찾기는 기본 접힌 `즐겨찾기` 그룹 아래에 모였다.
PR #4 병합 · 서명 자산 6개 공개 · 최종 설치본 실행까지 닫혔다.
...
```
브랜치   **main 에 PR #4가 squash merge됐다** (2026-09-07).
                 태그 v0.9.0은 병합 커밋 `e3fdd7a`를 가리킨다
작업트리 ✅ `Ctrl+A` 전체 선택 · 트리 자동 따라가기 제거 · 트리 툴바 `모두 접기` ·
                 즐겨찾기 그룹이 들어갔다. `TreeScroll`과 양방향 배선은 삭제됐다
버전     0.9.0   ✅ **v0.9.0 공개 릴리스 완료** (2026-09-07).
                 서명 자산 6개를 GitHub에 올렸고 이 기계 설치본은
                 `0.9.0+e3fdd7a…`이다. 창 표시와 새 오류 없음 확인.
                 그 앞은 0.8.5(외부 도구) · 0.7.0(다크모드) ·
                 0.6.1(창 결함 둘) · 0.6.0(분할).
                 ⚠ `Get-AuthenticodeSignature` 는 `UnknownError` 를 내는데, **서명은 붙어
                 있고 이 기계의 신뢰 루트에 자체 서명 인증서가 없어서**다 — 결함이 아니다
                 (`HISTORY.md` §v0.7.0)
테스트   2263 통과   Core 654 · Shell 394 · App 1120 · Host 95
게이트   fast (build -warnaserror · test --blame-hang · check-structure) ✅
         full (Release build -warnaserror) ✅ **2026-09-07 · PR #4 병합 직전 HEAD에서 넷 다
         다시 돌렸다. 테스트 2263 · 실패 0 · 경고 0.**
...
### v0.9.0 선택·폴더 트리 — **병합·공개 배포·재설치까지 닫혔다**

PR #4를 `e3fdd7a`로 squash merge했고 태그 v0.9.0과 GitHub 릴리스도 같은 커밋에 만들었다.
서명 자산 6개가 모두 업로드됐고 최종 설치본 `0.9.0+e3fdd7a…`이 실제로 뜬다.
Setup SHA-256은 `4A10E73CCA0D05AC09EDDE096B8F2A86D721930554790BEAD8B7286B1FD071C3`이다.
이슈 #1은 병합과 함께 닫혔다.

### 0. 2구간(v0.8.5) 외부 도구 — **병합·공개 배포·재설치까지 닫혔다**

게이트 4종 ✅ · 테스트 **2280** ✅. 전문은 `docs/PRD-v2.md` §21 · `ADR-022` · step 목록은
`phases/5-external-tools/index.json`. **PR #3을 `2211c0c`로 squash merge했고 태그 v0.8.5와
GitHub 릴리스도 같은 커밋에 만들었다.** 서명 자산 6개가 모두 업로드됐고 최종 설치본
`0.8.5+2211c0c…`이 실제로 뜬다. `error.log`에는 이번 실행의 새 항목이 없다.
...
- **트리 가로 스크롤을 껐다** (`Disabled`). 탐색기 탐색창에 없는 막대였다. `Hidden` 이
  아닌 이유는 `Hidden` 이 막대만 감추고 내용을 무한 폭으로 재기 때문이다. 깊은 노드가
  화면을 가로로 밀어도 사용자는 되돌릴 수단이 없었다.
