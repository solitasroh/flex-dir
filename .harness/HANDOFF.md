...
버전     0.8.2   릴리스는 v0.1.0 ~ v0.7.0 까지 나갔다. **v0.8.0~v0.8.2 는 패키지만
...
게이트   fast (build -warnaserror · test --blame-hang · check-structure) ✅
         full (Release build -warnaserror) ✅ **0.8.2 수정 뒤 다시 돌렸다 (2026-08-21).**
         ⚠ **게이트는 이 결함을 못 잡았다.** `MenuItem.Icon` 이 안 그려지는 것은 빌드도
         테스트도 구조 게이트도 통과한다 — 자동 채점의 경계다 (CLAUDE.md §5)
