...
```
브랜치   main  ·  117b1c2 까지 푸시됨. **§14(UI 스레드 예외)가 작업 트리에 미커밋으로 있다**
버전     0.4.1   릴리스 v0.1.0 ~ v0.4.1. **이 기계 설치본도 0.4.1 이다** (2026-08-11 아침에
                 설정 창의 '업데이트 확인' → 알림의 '지금 설치' 로 올렸다).
                 **§14 는 아직 릴리스에 없다** — 설치본은 여전히 예외 하나에 죽는다
테스트   1693 통과   Core 534 · App 775 · Shell 293 · Host 91
                 (+27: CrashNotice 17 · ErrorLog 9 · Workspace 1)
게이트   fast (build -warnaserror · test --blame-hang · check-structure) ✅
         full (Release build -warnaserror) ✅ **§14 를 넣고 4종 전부 다시 돌렸다**
         (2026-08-11). 확인용 저장소 Release 프로세스를 먼저 죽여야 통과한다
         (아래 §0.4.1 배포)
...
실물 확인 여섯 항목이 전부 닫혔다 (§14 §실물 확인). **다음 릴리스에 나가야 값이 생긴다** —
설치본은 아직 0.4.1 이라 예외 하나에 죽는다.
