...
버전     0.4.3   릴리스 v0.1.0 ~ v0.4.3. **v0.4.3 이 §15(마우스 보조 버튼)를 실어 나갔고**
                 ...
                 ⚠ **§16 수정(활성화 채널)은 커밋만 됐고 릴리스에 없다** — 다음 배포가 실어야
                 한다. 그때까지 설치본은 인자 없는 두 번째 실행 한 번에 귀를 닫을 수 있다
...
### 0. 탭 — 다음에 착수한다 (2026-08-11 · 사용자 결정)

사용자가 마우스 보조 버튼 확인 직후에 정했다: **"그 다음은 tab 기능 개발에 착수할거야."**
정본은 `docs/PRD-v2.md` §8 의 항목이고, 여기서는 **다음 세션이 첫 5분에 알아야 할 것**만
적는다.

- **범위는 정해지지 않았다.** 착수는 기획부터다 (`/harness:plan` 의 인터뷰) — 코드부터
  건드리면 안 된다. 갈림은 §8 항목에 나열해 뒀다 (페인마다 vs 창 하나 · 세션 복원 ·
  '활성 탭' 의 정의 · 닫은 탭 되살리기).
