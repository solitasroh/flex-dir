...
닫혔다** (2026-08-21 · 아래 §다음 작업 0-0). 2구간(v0.9.0)은 외부 도구다 (아래 §다음 작업).
...
버전     0.9.0   릴리스는 v0.1.0 ~ v0.7.0 까지 나갔다. **v0.8.0~v0.9.0 은 패키지만
                 굽는다** (`pack.ps1 -NoUpload -SignThumbprint` · 서명본).
                 ⚠ **0.9.0 은 아직 굽지 않았다** — `Directory.Build.props` 의 버전만
                 올렸다. 굽는 것은 사람 확인과 함께 사용자가 한다 (§배포 절차 ④).
...
게이트   fast (build -warnaserror · test --blame-hang · check-structure) ✅
         full (Release build -warnaserror) ✅ **2026-08-24 · v0.9.0 문서·버전과
...
### 0. 2구간(v0.9.0) 외부 도구 — 코드는 끝났다. **사람 확인 전**

게이트 4종 ✅ · 테스트 **2277** ✅ (2026-08-24). 전문은 `docs/PRD-v2.md` §21 · `ADR-022` ·
step 목록은 `phases/5-external-tools/index.json`. **버전은 `0.9.0` 으로 올렸고 아직 굽지
않았다** — 굽는 것은 사람 확인과 함께 사용자가 한다 (§배포 절차 ④).
...
1. **`C:\` 에서 `[>_]` 가 그 경로에서 열린다** — 사용자 지정 WezTerm 으로 밟는다:
   실행 파일 `wezterm-gui.exe` · 인자 `start --cwd "{path}"`.
2. **`\\10.10.10.23\home` 에서도 1번이 되고, `\\10.10.10.23` 에서는 둘 다 회색이다.**
   막히는 것은 서버 루트 하나여야 한다 (`IsNetworkServer` — `IsNetwork` 가 아니다).
3. **`[VS]` 가 현재 폴더를 연다.** 그리고 VS Code 를 지우거나 App Paths 키를 임시로
   옮긴 뒤 **창을 숨겼다 다시 보이면 비활성이 된다** (탐지는 창이 보일 때마다 돈다).
   ⚠ **확인 뒤 반드시 원상복구한다.**
4. **`[VS]`·`[>_]` 실루엣이 16px 실물에서 서로 구분되고**, 기존 계열(선 `E8A4`·`E8FD` ·
   사각 `E8A9`·`E15B` · 폴더 `E8B7` · 점 `E712`)과도 구분된다.
5. **다크·라이트 양쪽에서 두 아이콘 대비와 비활성 회색을 확인했다.**
   ⚠ `settings.json` 의 `theme` 을 잠시 바꿔 보고 **반드시 `Dark` 로 되돌린다**
   (사용자가 명시적으로 잡아 둔 값이다).
6. **창 900 언저리 4분할에서 외부 도구가 가장 먼저 접히고**, 이동·새로 고침·알려진
   폴더는 남는다. **4분할 최대화(페인 약 424)에서도 이제 접힌다** — 툴바 합이
   347 → 420 이 되어 임계값을 지났다 (§21 · **계산값이고 실측 전이다**).
7. **서명된 설치본이 실제로 뜨고 페인 우클릭에서 크래시하지 않는다.**
   ⚠ 우클릭은 합성 입력으로 못 밟는다 — 사람이 직접 누른다.
...
`theme` 되돌리기)가 거기 그대로 있다. **열린 항목은 하나뿐이다: 라이트 테마 대비** —
픽셀·대비는 자동 채점되지 않는다 (CLAUDE.md §5).
