...
```
브랜치   main  ·  **origin/main 은 d31118c — 푸시됐다** (2026-08-12).
                 푸시는 사용자 지시가 있을 때만 한다
작업트리 **깨끗하다** (2026-08-12). 분할 구간이 네 커밋으로 들어갔다 —
                 구현+테스트(`4f9e86a`) · `docs/`(`888f092`) · `.harness/`(`e8b2133`) ·
                 버전(`d31118c`). **구현과 테스트를 나누지 않았다**: `PaneSide` 를 없애면
                 Core·Shell·App·Host·테스트가 동시에 깨져서, 나누려면 고장난 중간 상태를
                 지어내야 한다
버전     0.6.1   릴리스 v0.1.0 ~ v0.6.1. **v0.6.1 이 최대화 잘림·트리 가로 스크롤
                 수정을 실어 나갔다** (2026-08-12 · 아래 §다음 작업 0). 그 앞의 v0.6.0 이
                 분할(§18)을 나갔다 (`HISTORY.md` §v0.6.0).
                 ⚠ **v0.6.0 만 자동 업데이트 경로를 안 밟았다** — 0.5.0 이 새 저장 형식을
                 못 읽어 기억을 덮어쓰기 때문이다. `Setup.exe` 로 직접 올렸다.
                 **0.6.1 은 알림 경로로 간다** (같은 포맷이라 막을 이유가 없다)
                 ⚠ **v0.6.1 은 무서명으로 나갔다.** 서명 배선은 0.6.1 커밋 뒤에 들어갔다
                 (`e0b2c03`) — 0.6.2 부터 서명본이다
...
게이트   fast (build -warnaserror · test --blame-hang · check-structure) ✅
         full (Release build -warnaserror) ✅ **0.6.1 커밋 뒤 다시 돌렸다.**
...
### 0-3. 다크모드 — 구현·게이트·실물 확인까지 끝났다. 배포만 남았다 (2026-08-12)
...
다크모드           ◐  §19 · ADR-020 — 테마 3택·OS 추종·기본 크롬 재도색. 포트 16/16.
                      게이트 4종 ✅ · **실물 확인 여덟 ✅** (2026-08-12).
                      게이트 통과 뒤 실물에서 결함 셋이 나왔고 전부 고쳤다.
                      **배포만 남았다** — 위 §다음 작업 0-3
