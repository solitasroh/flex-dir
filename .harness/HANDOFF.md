...
작업트리 알려진 폴더 메뉴의 **shell 항목 아이콘**(v0.8.1)이 **step 단위 커밋**으로
                 들어갔고, 그 위에 **v0.8.2 수정 커밋**(`03106f2`)이 얹혔다 —
                 **v0.8.1 은 결함본이다**: 아이콘을 `MenuItem.Icon` 에 걸었는데 이 앱의
                 암시적 `MenuItem` 템플릿에 그 프리젠터가 없어 메뉴가 글자만 떴다.
                 게이트 4종을 다 통과하고 서명까지 붙은 채로 그랬다 — **실물 확인이
                 아니면 못 잡는다** (`HISTORY.md` §v0.8.2 · `PRD-v2.md` §20 §값을 치르고).
                 step 목록의 정본은 `phases/4-known-folder-icons/index.json` 이고
                 그 앞 1구간은 `phases/3-toolbar-overflow/index.json`
버전     0.8.4   릴리스는 v0.1.0 ~ v0.7.0 까지 나갔다. **v0.8.0~v0.8.4 는 패키지만
                 굽는다** (2026-08-21 · `pack.ps1 -NoUpload -SignThumbprint` · 서명본).
...
게이트   fast (build -warnaserror · test --blame-hang · check-structure) ✅
         full (Release build -warnaserror) ✅ **0.8.3 수정 뒤 다시 돌렸다 (2026-08-21).**
         ⚠ **게이트가 이 구간에서 화면 결함 둘을 연속으로 통과시켰다** — `MenuItem.Icon` 이
