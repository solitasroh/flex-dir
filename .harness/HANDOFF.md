...
**이 세션에 들어온 것**: v0.3.1 릴리스 · **설정 창**(정보·시작 폴더·숨김 파일) ·
**감시 백오프** · **트리가 활성 페인을 따라간다**. 넷 다 실물로 확인했다.
...
```
브랜치   main  ·  06b65d3 까지 푸시됨. 그 뒤 설정 창·감시 백오프·트리 따라가기는
                 **아직 커밋되지 않았다** (사용자 요청 대기)
버전     0.3.1   릴리스 v0.1.0 ~ v0.3.1. **이 기계에는 아직 0.1.2 가 설치돼 있다**
                 (0.3.1 을 냈으므로 다음 실행이 74MB 를 받는다 — 사내망에서 약 15분)
테스트   1658 통과   Core 534 · App 749 · Shell 293 · Host 82
...
### 1. 사람이 손으로 볼 것 — 이번 세션이 만든 것

전부 게이트를 통과했고 자동으로 닿는 데까지는 확인했다. 남은 것은 **조작감**이다.

| 확인 | 왜 사람이 봐야 하나 |
|---|---|
| 설정 패널의 생김새·간격 | 픽셀은 자동 채점되지 않는다 (CLAUDE.md §5) |
| 트리 따라가기의 **체감** | `C:\` 처럼 형제가 많은 폴더를 지날 때 화면이 튀지 않는가 |
