...
자리가 있다는 것이다. **v0.4.3 으로 나갔고 사용자가 실물에서 확인했다** (*"정상동작해"* ·
아래 §0.4.3 배포). **지금 저장소의 끝은 0.4.3 이고 열린 확인은 없다.**

**그리고 배포 뒤 게이트가 오래된 부채 하나를 잡았다** — `docs/PRD-v2.md` §7 이 *"원인 미확정"*
으로 열어 둔 **Host 테스트 간헐 실패**가 `SendAsync_NoArguments_StillReachesThePrimary` 였고,
**테스트 결함이 아니라 제품 결함**이었다 (§16). 인자 없는 두 번째 실행이 붙자마자 끊으면
상주 프로세스의 활성화 반복자가 `IOException` 으로 죽어 **그 뒤로 두 번째 실행이 창을 못
띄운다** — 그리고 그 예외는 `_ = router.RunAsync(...)` 라 **아무 곳에도 닿지 않는다**
(§14 의 핸들러도 UI 스레드가 아니라 못 본다). 재현 테스트를 먼저 세우고 고쳤다.
**이 수정은 아직 배포되지 않았다 — 0.4.3 에는 없다.**

**탭이 화면까지 섰다** (2026-08-11 · `docs/PRD-v2.md` §17). 소유 구조 · 저장 포맷 ·
페인 간 소유권 이동에 이어 **탭 줄 XAML · 키 일곱 · 컨텍스트 메뉴 일곱 · 드래그**까지
들어왔다. `*.xaml` 이 이 기능에서 처음으로 바뀌었다. 아래 §다음 작업 0 참조.

**그리고 화면이 서자마자 그것이 결함 셋을 청구했다** — 활성 탭이 스크롤로 끌려오지 않던 것 ·
'반대편 페인으로 보내기' 가 눌려도 아무 일이 없던 것 · **활성 탭이 아예 구분되지 않던 것**
(마지막은 사용자가 실물에서 잡았다). 앞의 둘은 **조용히 죽은 바인딩**이고 셋째는 **근거를
잃은 채 남아 있던 스펙 값**이다 — 셋 다 화면이 없을 때는 존재할 수 없는 종류였다.
전문은 §17 §값을 치르고 배운 것 (탭 줄 화면 구간).

**그리고 사람이 그것을 밟았다 — 탭은 실물에서 닫혔다** (2026-08-12). 사용자가 아홉 항목을
확인했고 전부 정상이었다. **그중 넷은 기계가 한 번도 못 밟던 것**이다 (드래그 둘 · 보내기
메뉴 · 제목 이름 바꾸기). 남은 하나(*"고정 탭이 실제로 쓰이는가"*)는 며칠 써야 보이는
관측 항목이라 배포를 막지 않았다. **그리고 같은 날 나갔다 — v0.5.0 이 탭과 §16 을 함께
실어 나갔고, 이 기계 설치본에서 도는 것까지 확인했다** (아래 §0.5.0 배포).
...
```
브랜치   main  ·  **origin/main 은 0dead60 — 푸시됐다** (2026-08-12).
                 푸시는 사용자 지시가 있을 때만 한다
작업트리 **깨끗하다** (2026-08-12). 탭 줄 화면 구간이 다섯 커밋으로 들어갔다 —
                 구현(새 파일 8 + 수정 6) · `docs/` · `.harness/` · 결정 둘 · 버전.
                 **확인이 잡은 결함 셋은 구현 커밋에 섞여 있다** — 셋 다 새로 추가된
                 XAML 블록 *안쪽*의 한 속성·한 `Border` 라 원본과 분리되지 않는다
                 (나누려면 고장난 중간 상태를 지어내야 한다)
버전     0.5.0   릴리스 v0.1.0 ~ v0.5.0. **v0.5.0 이 탭과 §16(활성화 채널)을 함께 실어
                 나갔다** (2026-08-12 · 아래 §0.5.0 배포). **이 기계 설치본도 0.5.0 이고
                 실물에서 도는 것을 확인했다.** §16 의 경고는 이제 없다 — 인자 없는 두
                 번째 실행이 활성화 채널을 죽이던 결함이 배포까지 닫혔다
테스트   1893 통과   Core 545 · App 955 · Shell 299 · Host 94
                 (탭이 +186. ViewModel·Core 구간 135, 탭 줄 화면 구간 51 —
                  `Views/Tab*Tests.cs` 넷 + 순서 변경·보내기·제목 편집·활성 탭 판정)
게이트   fast (build -warnaserror · test --blame-hang · check-structure) ✅
         full (Release build -warnaserror) ✅ **둘 다 2026-08-11 저녁 미커밋 상태에서
         다시 돌렸다.** 새 테스트는 부하 아래 3회 반복해도 안정이다
         ⚠ **저장소 Debug 빌드로 앱을 띄우면 산출물을 잠근다** — 확인이 끝나면 반드시
         죽인다. 설치본은 잠그지 않는다 (아래 §0.4.1 배포의 경고)
         ⛔ 도그푸딩 게이트는 없다 — 스크립트를 지웠다 (ADR-007 §폐기)
phases/  0-core-model · 1-core-pipeline · 2-viewmodel 모두 completed
```

### 0.5.0 배포 — 끝났다 (2026-08-12 · 탭과 §16 을 함께 실어 나갔다)
...
**그리고 사용자가 4·5번을 눌러 확인했다** — *"정상동작해"* (`docs/PRD-v2.md` §15
§실물 확인). 곧 이 기계는 `WM_XBUTTONDOWN` 으로 오는 마우스이고, `WM_APPCOMMAND` 훅은
넣을 이유가 없다. **이 배포에 열린 확인은 남지 않았다.**
...
### 0. 탭 — 끝났다. v0.5.0 으로 나갔고 실물에서 돈다 (2026-08-12 갱신)
...
**그리고 그 릴리스가 나갔다 — v0.5.0** (2026-08-12 · 위 §0.5.0 배포). §16(활성화 채널)도
함께 실어 나갔다 (사용자 결정 2026-08-11). 이 기계 설치본이 0.5.0 으로 교체되고 탭 줄이
그려진 것까지 확인했다. **탭에 열린 것은 없다.**

**그러므로 다음 축은 도그푸딩이 고른다** (ADR-007 §폐기의 자리). 지금 알고 있는 후보 셋:

| 후보 | 근거 |
|---|---|
| **9P(WSL) 감시** | `자동 갱신이 느려졌습니다 — 새로 고침(F5)` 을 사용자가 실제로 보고 거슬린다고 했다. 이론상의 미결이 아니라 매일 보이는 자국이다 (docs/PRD-v2.md §13 마지막) |
| **고정 탭 vs 즐겨찾기** | 겹치는 자리를 관측 대상으로 두었다 (§17 §겹치는 자리). 한쪽만 쓰이면 다른 쪽을 덜어낼 근거가 생긴다 — **탭의 유일한 열린 항목이다** |
| **델타 패키지** | 갱신마다 74MB 다. 이번에 받는 데 8분 걸렸다 — 세 번째 값이고, 편차가 크다는 것이 확정됐다 |

<details>
<summary>이전 기록 — 탭 줄 화면 구간이 선 직후 (2026-08-11 저녁)</summary>

### 0-0. 탭 — 화면까지 섰다. 남은 것은 사람이 만져 보는 것뿐이다 (2026-08-11 저녁)
...
<details>
<summary>이전 기록 — ViewModel·Core 구간 (2026-08-11 낮)</summary>

### 0-1. 탭 — ViewModel·Core 는 끝났다. 남은 것은 화면과 드래그다 (2026-08-11)
...
- **델타 패키지를 만들지 않는다.** 갱신마다 74MB 를 받는다. **걸리는 시간은 때에 따라
  ...
  **진행을 `.partial` 파일 크기로 재려 하면 안 된다** (버퍼 때문에 점프한다 — §0.5.0 배포).
