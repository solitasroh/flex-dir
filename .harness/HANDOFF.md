...
**세 요청이 남아 있다** (아래 §다음 작업): 설정 창 · 그 안의 버전 정보 · 숨김/시스템 파일
보기. 범위는 사용자와 이미 정해 두었다.
...
```
브랜치   main  ·  b5e142d 까지 푸시됨. 그 뒤 커밋 셋은 로컬에만 있다
버전     0.3.0   릴리스 v0.1.0 ~ v0.3.0. **이 기계에는 아직 0.1.2 가 설치돼 있다**
                 (0.2.0 알림까지 봤고 설치는 하지 않았다)
테스트   1532 통과   Core 508 · App 665 · Shell 284 · Host 75
...
2026-08-10 에 셋이 늘었다. 전부 트리와 즐겨찾기가 요구한 것이다 (docs/PRD-v2.md §10·§10-2).
...
### 1. 설정 창 — 범위는 정해졌고 아직 착수하지 않았다

사용자 결정 (2026-08-10). 세 항목만 넣는다. 밀도 옵션(24/28)은 이번에 제외했다.

| 항목 | 메모 |
|---|---|
| 정보 | 버전 · **업데이트 수동 확인** · 상태 폴더 열기. 버전 표시 자리는 여기다 (사용자 결정) |
| 시작 폴더 | 마지막 폴더 / 정해 둔 폴더. `WorkspaceViewModel.RestoreAsync` 의 폴백과 이어진다 |
| 숨김·시스템 파일 보기 | **Shell 을 건드릴 필요가 없다** — 아래 참조 |

**숨김 필터는 이미 UI 정책으로 설계돼 있다.** `FileSystemFolderSource` 가
`AttributesToSkip = 0` 으로 두고 *"보여줄지 말지는 UI 정책이고, 열거는 있는 것을 다
내야 한다"* 고 적어 놨는데 **정작 아무도 필터링하지 않는다** — 그래서 트리에
`$Recycle.Bin`·`System Volume Information` 이 보인다. ViewModel 필터 하나면 된다.

설정 저장은 **뷰 상태와 다른 파일**이 맞다 — 즐겨찾기(`favorites.json`)와 같은 논리다.
설정은 사용자 의도라 캐시처럼 조용히 기본값으로 접으면 안 된다.
