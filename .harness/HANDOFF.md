...
**다음은 phase B — View** 이고, **착수 전에 `docs/DESIGN.md` §9 를 채워야 한다**(아래 §다음 작업).
...
### 선행조건: `docs/DESIGN.md` §9 의 남은 셋

§9 는 **체크 안 된 항목 둘짜리 자리표시자**다(`docs/DESIGN.md:122-128`). 다만 거기 적힌
상호작용 상태 셋 중 **스플리터 드래그 중은 목업에 이미 있다**(`v1-two-pane.html:44-46,
240-244` — hover/drag accent 바 + 최소 폭 320 클램프). 실제로 비어 있는 것은 아래 셋이다.

- [ ] **키보드 맵 표** — 목업에 키 조합이 하나도 없다(`title` 에 단축키 힌트도 없다).
      `docs/UI_GUIDE.md` §키보드 의 최소선을 실제 키 조합으로 + 탐색기 기본 단축키 충돌 검토.
      **ViewModel 커맨드는 이미 다 있다** — 남은 것은 XAML `InputBindings` 의 제스처 매핑뿐이다.
- [ ] **이름변경 인라인 편집** — 목업에 없다. `PaneViewModel.RenamingName` 에 대응하는
      시각이 정해져 있지 않다.
- [ ] **페인 간 드래그앤드롭** — 목업에 없다. 드래그 중 표시 · 드롭 대상 표시 ·
      복사/이동 구분이 전부 미정이다.
...
- [ ] **`docs/DESIGN.md` §9** — 키보드 맵 · 이름변경 인라인 편집 · 페인 간 드래그앤드롭.
      **phase B 의 선행조건.** (스플리터 드래그는 목업에 이미 있다.)
- [ ] **뷰 3종의 가상화를 어떻게 할 것인가 — phase B 의 가장 큰 기술 리스크.**
      `docs/ARCHITECTURE.md` §5 는 `VirtualizingStackPanel` + `Recycling` +
      `ScrollUnit="Item"` 을 **필수**로 못박았는데, 목업의 뷰 4종 중 **셋이 wrap 레이아웃**이다 —
      목록(다단 세로 흐름, 가로 스크롤) · 타일(grid auto-fill 220) · 큰 아이콘(grid auto-fill 116).
      **WPF 가 기본 제공하는 가상화 패널은 `VirtualizingStackPanel` 하나이고
      `WrapPanel`·`UniformGrid` 는 가상화하지 않는다.** CSS 에서 되는 것과 WPF 에서 가상화가
      유지되는 것은 다른 문제이므로, 목업이 있다고 해결된 것이 아니다.
      전작이 성능 때문에 `LVS_OWNERDATA` 를 골랐다가 뷰 모드를 Details 하나로 잃은 자리와
      같은 지점이다 (ADR-002).
      - 권장: **합성 행** — 한 줄에 N개를 담은 행 항목을 만들어 세로만
        `VirtualizingStackPanel` 로 가상화하고, 창 폭이 바뀌면 N 을 다시 계산한다.
        외부 의존 없이 §5 를 문자 그대로 지킨다. 대가는 App 쪽에 표시용 그룹 계층이 느는 것.
      - 대안: 커스텀 `VirtualizingPanel` 직접 구현 (정공법이나 난이도가 높다).
      - 기각: 가상화 포기 — 10만 항목에서 죽는다. 성능은 전작이 유일하게 성공한 축이다.
