...
>
> 갱신: 2026-09-07. 함께 볼 것: [`manual-plan.md`](./manual-plan.md) — 수동 phase 계획과
...
**v0.9.1은 마우스 선택과 새 탭 설정을 바로잡았다** (2026-09-07). 목록 빈 공간 드래그로
선택 사각형을 쓰고 `Shift` 추가·`Ctrl` 반전을 지원한다. 새 탭은 설정의 시작 폴더 규칙을
따른다. PR #5 병합 · 이슈 #2 닫힘 · 서명 자산 6개 공개 · 최종 설치본 실행까지 닫혔다.
...
```
브랜치   **main 에 PR #5가 squash merge됐다** (2026-09-07).
                 태그 v0.9.1은 병합 커밋 `05d4f2e`를 가리킨다
작업트리 ✅ 마우스 선택 사각형 · 새 탭 시작 폴더 설정 동기화가 들어갔다
버전     0.9.1   ✅ **v0.9.1 공개 릴리스 완료** (2026-09-07).
                 서명 자산 6개를 GitHub에 올렸고 이 기계 설치본은
                 `0.9.1+05d4f2e…`이다. 창 표시와 새 오류 없음 확인.
                 그 앞은 0.9.0(선택·트리 단순화) · 0.8.5(외부 도구) · 0.7.0(다크모드) ·
                 0.6.1(창 결함 둘) · 0.6.0(분할).
                 ⚠ `Get-AuthenticodeSignature` 는 `UnknownError` 를 내는데, **서명은 붙어
                 있고 이 기계의 신뢰 루트에 자체 서명 인증서가 없어서**다 — 결함이 아니다
                 (`HISTORY.md` §v0.7.0)
테스트   2273 통과   Core 654 · Shell 394 · App 1130 · Host 95
게이트   fast (build -warnaserror · test --blame-hang · check-structure) ✅
         full (Release build -warnaserror) ✅ **2026-09-07 · PR #5 병합 직전 HEAD에서 넷 다
         다시 돌렸다. 테스트 2273 · 실패 0 · 경고 0.**
