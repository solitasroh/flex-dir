...
관측 항목이다. **다음 일은 릴리스이고, 그 배포가 §16 도 함께 실어 나른다.**
...
```
브랜치   main  ·  origin/main 은 8470b00. **미푸시 커밋 8개**
                 (탭 기획 1 + 구현·문서 4 + 탭 줄 화면 3). 푸시는 사용자 지시가 있을 때만 한다
작업트리 **깨끗하다** (2026-08-12). 탭 줄 화면 구간이 세 커밋으로 들어갔다 —
                 구현(새 파일 8 + 수정 6) · `docs/` · `.harness/`.
                 **확인이 잡은 결함 셋은 구현 커밋에 섞여 있다** — 셋 다 새로 추가된
                 XAML 블록 *안쪽*의 한 속성·한 `Border` 라 원본과 분리되지 않는다
                 (나누려면 고장난 중간 상태를 지어내야 한다)
버전     0.4.3   릴리스 v0.1.0 ~ v0.4.3. **v0.4.3 이 §15(마우스 보조 버튼)를 실어 나갔고**
                 사용자가 실물에서 확인했다. **이 기계 설치본도 0.4.3 이다**
...
### 0. 탭 — 사람 확인이 닫혔다. 남은 것은 릴리스뿐이다 (2026-08-12 갱신)
...
**그러므로 다음 일은 릴리스다** — 이 배포가 §16(활성화 채널)도 함께 실어 나른다
(사용자 결정 2026-08-11). 순서는 위 §0.4.1 배포 의 넷을 그대로 밟는다:
상주 프로세스 죽이기 → 게이트 → `Directory.Build.props` 버전 올림 → 푸시 · `scripts/pack.ps1`.
