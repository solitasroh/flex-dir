...
**v0.9.2는 창이 뜨지 않던 두 원인을 닫았다** (2026-09-16). 저장 배치가 어느 모니터에도
없으면 보이는 순간 주 모니터 가운데로 옮기고(`ResidentWindow.EnsureOnScreen`), 창 표시가
복원의 폴더 열기(끊긴 NAS 드라이브 목록 포함)를 기다리지 않는다(`RestoreAsync` 콜백).
`main` 직접 커밋 셋 · 서명 자산 6개 공개 · 최종 설치본 실행까지 닫혔다. **"안 켜진다" 는
먼저 `perf.log` 를 본다** — `WindowShown` 이 찍히면 Show 는 됐고 좌표가 문제다;
`ColdStart` 와 `WindowShown` 의 시각 차가 복원 대기다.
