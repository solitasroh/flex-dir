...
### 0. 2구간(v0.8.5) 외부 도구 — **나갔다. 사람 확인 일곱 중 다섯이 닫혔다**
...
4. **`[VS]`·`[>_]` 실루엣이 16px 실물에서 서로 구분되고**, 기존 계열(선 `E8A4`·`E8FD` ·
   사각 `E8A9`·`E15B` · 폴더 `E8B7` · 점 `E712`)과도 구분되는가.
   두 아이콘은 **채우기**라 계열이 셋째로 갈린다 — 대신 기존 외곽선 글리프보다 무겁다.
5. **다크·라이트 양쪽 대비와 비활성 회색.** 양쪽 캡처는 떴다(다크·라이트 각각 툴바
   확대). 픽셀·대비는 자동 채점되지 않는다 (CLAUDE.md §5).
   ⚠ 확인하려고 `theme` 을 `Light` 로 바꿨다가 **`Dark` 로 되돌려 놓았다.**

**그리고 7번의 나머지 반**: **페인 우클릭은 아직 안 밟았다** (§10 의 `StaticResource`
크래시 자리). ⚠ **합성 입력으로 못 밟는다 — 사람이 직접 누른다.**
