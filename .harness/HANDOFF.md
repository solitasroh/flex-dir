...
**v0.4.2 로 나갔다** (아래 §0.4.2 배포). **지금 저장소의 끝은 0.4.2 다.**
...
```
브랜치   main  ·  ee997f4 까지 푸시됨. 미푸시 커밋은 없다
버전     0.4.2   릴리스 v0.1.0 ~ v0.4.2. **v0.4.2 가 §14(UI 스레드 예외)를 실어 나갔다**
                 (2026-08-11 · 아래 §0.4.2 배포). **이 기계 설치본은 아직 0.4.1 이고
                 0.4.2 를 받아 둔 채 알림 바가 떠 있다** — '지금 설치' 는 사용자가 고른다
...
게이트   fast (build -warnaserror · test --blame-hang · check-structure) ✅
         full (Release build -warnaserror) ✅ **0.4.2 배포 직전에 돌렸다** — 확인용 저장소
         Release 프로세스를 먼저 죽여야 통과한다 (아래 §0.4.1 배포)
